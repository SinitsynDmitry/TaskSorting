namespace RowsSorter
{
    internal class ReadOnlyMemoryByteComparer : IComparer<ReadOnlyMemory<byte>>
    {
        ...
        private static (ReadOnlyMemory<byte> firstSegment, ReadOnlyMemory<byte> secondSegment) SplitByteArray(ReadOnlyMemory<byte> data,byte separator = (byte)'.')
        {
            int separatorIndex = data.Span.IndexOf(separator);
