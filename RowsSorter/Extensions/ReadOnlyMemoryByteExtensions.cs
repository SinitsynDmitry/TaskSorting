...
public static class ReadOnlyMemoryByteExtensions
{
    ...
    public static void SplitIntoLines(this ReadOnlyMemory<byte> chunkBytes, List<ReadOnlyMemory<byte>> lines, byte separator = (byte)'\n')
    {
        int position = 0;
        ...
        {
            int separatorIndex = span[position..].IndexOf(separator);

            if (separatorIndex == -1)
            {
                lines.Add(chunkBytes.Slice(position));
                break;
            }

            lines.Add(chunkBytes.Slice(position, separatorIndex + 1));
